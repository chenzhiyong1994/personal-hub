      ...
      <div className="wrap">
        <Reveal>
          <header className="section-head">
            ...
            <h2 className="section-head__title">工作系统</h2>
            <span className="section-head__en">My Daily Systems — Built on AI Agents</span>
          </header>
        </Reveal>
        ...
        <Reveal>
          <div className="systems__intro">
            <p>
              还有一些东西，没必要包装成 App。它们更像我每天使用的一套幕后班底：Agent 先查资料、整理草稿、
              盯住前后矛盾，我来决定写什么、信什么、最后交出什么。用久了，就慢慢长成了这五套工作流。
            </p>
        ...
        <div className="system-list">
          {workSystems.map((system, systemIndex) => (
            <Reveal key={system.id} delay={systemIndex * 0.04}>
              <article
                className="system-card"
                style={{ ["--system-accent" as string]: ACCENTS[system.accent] }}
              >
                ...
                <div className="system-flow" aria-label={`${system.name}流程`}>
                  {system.flow.map((step, index) => (
                    <div className="system-flow__step" key={step}>
                      <span>{String(index + 1).padStart(2, "0")}</span>
                      <b>{step}</b>
                      {index < system.flow.length - 1 && <i aria-hidden="true">→</i>}
                    </div>
                  ))}
                </div>
                ...
                {system.lineage && (
                  <footer className="system-card__lineage">
                    <span>从这些早期实验长出来</span>
                    <div>{system.lineage.map((item) => <b key={item}>{item}</b>)}</div>
                  </footer>
                )}
              </article>
            </Reveal>
